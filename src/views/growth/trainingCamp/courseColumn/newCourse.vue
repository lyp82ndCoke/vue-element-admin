<template>
  <div class="app-container">
    <sticky class-name="sub-navbar">
      <el-col :span="10" class="subtitle">新建课程</el-col>
    </sticky>
     <el-form ref="form" :model="form" class="form-wrap" label-width="140px">
      ...
      <el-form-item label="是否启用开营日期" v-if="form.type == '2'">
        ...
        <el-col :span="18" v-if="form.is_enable == '1'">
          启用开营日期，课程解锁按照开营日期+1为准，启动后课程类型会更换为赠送课程类型
        </el-col>
      </el-form-item>
      <el-form-item label="开营日期" v-if="form.is_enable == '1'">
        <el-col :span="5">
...
export default {
  ...
  created() {
    ...
    if(this.$route.params.Courseid != '0'){
      this.form.spec_column_id = this.$route.params.Courseid;
      specColumnDetails({spec_column_id:this.$route.params.Courseid}).then(res => {
        if (res.code == 200) {
  ...
  methods: {
    onSubmit() {
      ...
      if(options.spec_column_title == ''){
        ...
        if(options.audio_url.indexOf('mp4') == -1){ 
          options.type = 1 
        }else{
          options.type = 2
        }        
