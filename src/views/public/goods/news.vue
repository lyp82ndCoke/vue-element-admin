    ...
    >
      <el-row>
        <el-col :span="12">
          ...
          <el-form-item label="商品分类：" prop="item_type">
            <el-select v-model="form.item_type" clearable placeholder="请选择商品分类">
              <el-option
                v-for="item in [{label:'训练营(普通)',value:9},{label:'训练营(需激活)',value:8},{label: '普通商品',value:1}]"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          <el-form-item label="购买限制：" prop="astrict_type">
            <el-select v-model="form.astrict_type" placeholder="请选择购买限制">
              <el-option
    ...
    reBuild: function (val, oldVal) {
      ...
      for (let i = 0; i < ret.length; i++) {
        let sku = {
          ...
          astrict_type: this.form.astrict_type
        }
        sku.values = []
        ret[i].forEach(function (item) {
          // console.log('哈哈哈item:', item)
          sku.values.push({ sku_vid: item.sku_vid, sku_v: item.sku_v, sku_kid: item.sku_kid, sku_k: item.sku_k })
        })
        if (oriSkus.length >= ret.length) {
