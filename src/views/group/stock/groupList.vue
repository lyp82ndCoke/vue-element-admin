    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit size="small"
      ...
      <el-table-column align="center" prop="created_at" width="100" label="操作">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click="goSku2(scope.row.item_id,scope.row.sku1_kid)">查看详情</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 查看详情
    goSku2(item_id, sku1_kid) {
      this.$router.push({ path: './details', query: { item_id, sku1_kid } })
    },
    // 添加群显示
