<template>
  <div class="app-container">
    ...
    <el-form class="demo-form-inline" ref="form" :rules="rules" :model="form" label-width="160px" size="small">
      <el-row>
        <el-col :span="12">
          <el-form-item label="商品名：" prop="item_name">
            <el-input v-model="form.item_name" placeholder="请输入商品名"></el-input>
          </el-form-item>
          <el-form-item label="是否有群服务：" prop="chatroom_flag">
            <el-select v-model="form.chatroom_flag" placeholder="请选择是否有群服务">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="是否填写手机号：" prop="mobile_flag">
            <el-select v-model="form.mobile_flag" placeholder="请选择是否填写手机号">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="是否填写孩子性别" prop="sex_flag">
            <el-select v-model="form.sex_flag" placeholder="请选择是否填写孩子性别">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="是否填写孩子关系" prop="relation_flag">
            <el-select v-model="form.relation_flag" placeholder="请选择是否填写孩子关系">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="是否填写孩子年龄" prop="age_flag">
            <el-select v-model="form.age_flag" placeholder="请选择是否填写孩子年龄">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="是否填写生日" prop="birthday_flag">
            <el-select v-model="form.birthday_flag" placeholder="请选择是否填写生日">
              <el-option v-for="item in [{label:'是',value:1},{label:'否',value:0}]" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="分享描述：" prop="item_desc">
            <el-input v-model="form.item_desc" placeholder="请输入分享描述"></el-input>
          </el-form-item>
          <el-row>
            <el-col class="grid-content bg-putple-light">
              <el-form-item label="商品图片：" prop="pic_url" class="avater-wrap">
                <el-col>
                  <el-upload class="avatar-uploader" :action="host" :show-file-list="false"
                    :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                    <img v-if="form.pic_url" :src="form.pic_url" class="avatar" v-model="form.pic_url">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="现价(分)：" prop="price">
                <el-input v-model="form.price" onkeyup="value=value.replace(/[^\d]/g,'')" placeholder="请输入商品现价">
                </el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item label="原价(分)：" prop="original_price">
                <el-input v-model="form.original_price" onkeyup="value=value.replace(/[^\d]/g,'')"
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="轮播图：" required>
                <span class="banner-img-wrap" v-for="(item,index) in form.slider_img" :key="index">
                  <img class="banner-img" :src="item.img" alt>
                  <span class="banner-img-remove">
                    <i class="el-icon-delete" @click="delBannerImg(index)"></i>
                  </span>
                </span>
                <el-upload class="upload-demo" :show-file-list="false" :action="host" :on-preview="handlePreview"
                  :on-remove="removeImg" :file-list="fileList" :on-success="updateImg" list-type="picture-card"
          ...
          <el-row>
            <el-form-item label="商品编码：" prop="item_no">
              <el-input v-model="form.item_no" placeholder="请输入商品编码"></el-input>
            </el-form-item>
          </el-row>
        </el-col>
        <!-- <el-col :span="12">
      ...
      <el-form-item>
        <el-button type="primary" @click="onGoodSubmit('form')" :loading="saveGoods">更新商品基本信息</el-button>
      </el-form-item>
    ...
    <el-form class="demo-form-inline" ref="formSpec" :rules="formSpecs" :model="formSpec" label-width="160px"
      size="small">
      <el-row>
          ...
          <el-row v-for="(item, index) in specMore" :key="index">
            ...
            <el-row style="margin-top:20px;">
              ...
              <el-col :span="6">
                ...
                <i class="el-icon-close close-project" @click="removeProject(index)"></i>
              </el-col>
            </el-row>
            <!-- 规格值 -->
            ...
            <el-row style="margin-top:20px;">
              ...
              <el-col :span="5" v-for="(valueItem,valueIndex) in specMore[index].value" :key="valueIndex" style="margin-top:20px;">
                <!-- {{valueItem + 'valueItem'}} -->
                ...
                <i class="el-icon-close close-project" @click="removeValue(index, valueIndex)"></i>
              </el-col>

              <el-col :span="2">
                <el-button type="text" @click="addSpecValue(index)" style="margin-top:20px;">添加规格值</el-button>
              </el-col>
            </el-row>
          </el-row>
          <el-row style="margin-top:20px;">
            <el-button type="primary" v-if="specMore.length<=2" size="mini" @click="addSpecProject">添加规格项目</el-button>
          </el-row>
        </el-form-item>
      </el-row>
      <el-form-item v-if="tableData.length" label="规格明细：" required>
        <table class="table-sku" border="1px solid #ccc" id="getSkuk">
          <thead>
            <tr>
              <td v-for="(list,index) in tableData" :key="`list${index}`">{{list['sku_k']}}</td>
          ...
          <tbody>
            ...
            <tr v-for="(sku,index) in formSpec.skus" :key="index">
              ...
              <td>
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.sku_price`" :rules="formSpecs.sku_price">
                    <!-- :prop="`skus.${index}.sku_price`" -->
                    <el-input size="small" type="text" v-model="sku.sku_price"
                      onkeyup="value=value.replace(/[^\d]/g,'')" maxlength="9"></el-input>
                  </el-form-item>
                </el-col>
              </td>
              <!-- 原价 -->
              <td>
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.sku_original_price`" :rules="formSpecs.sku_original_price">
                    <!-- :prop="`skus.${index}.sku_price`" -->
                    <el-input size="small" type="text" v-model="sku.sku_original_price"
                      onkeyup="value=value.replace(/[^\d]/g,'')" maxlength="9"></el-input>
                  </el-form-item>
                </el-col>
              </td>
              <!-- 库存 -->
              <td>
                <el-col v-if="!form.chatroom_flag" :span="24">
                  <el-form-item :prop="`skus.${index}.sku_with_hold_quantity`"
                    :rules="formSpecs.sku_with_hold_quantity">
                    <el-input type="text" v-model="sku.sku_with_hold_quantity" placeholder="请在库存管理中设置" maxlength="9">
                    </el-input>
                  </el-form-item>
                </el-col>
                <el-col v-else>
                  <el-form-item>
                    <el-input type="text" disabled v-model="sku.sku_with_hold_quantity" maxlength="9"></el-input>
                  </el-form-item>
                </el-col>
              </td>
              <!-- 商品编码 -->
              <td>
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.sku_item_no`" :rules="formSpecs.sku_item_no">
                    <!-- :prop="`skus.${index}.sku_item_no`" -->
                    <el-input size="small" type="text" v-model="sku.sku_item_no" maxlength="9"></el-input>
                  </el-form-item>
                </el-col>
              </td>
              <!-- 成本价 -->
              <td>
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.sku_cost_price`" :rules="formSpecs.sku_cost_price">
                    <!-- :prop="`skus.${index}.sku_cost_price`" -->
                    <el-input size="small" type="text" v-model="sku.sku_cost_price"
                      onkeyup="value=value.replace(/[^\d]/g,'')" maxlength="9"></el-input>
                  </el-form-item>
                </el-col>
              </td>
              ...
              <td>
                ...
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.item_id_xiao`" :rules="formSpecs.item_id_xiao">
                    <el-select v-model="sku.item_id_xiao" placeholder="请选择">
                      <el-option v-for="widItem in widgetId" :key="widItem.id" :label="widItem.goodsTitle"
                        :value="widItem.id"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
              </td>
              ...
              <td>
                <el-col :span="24">
                  <el-form-item :prop="`skus.${index}.shelf_flag`" :rules="formSpecs.shelf_flag">
                    <el-select v-model="sku.shelf_flag" placeholder="请选择">
                      <el-option v-for="shelfItem in shelf" :key="shelfItem.value" :label="shelfItem.label"
                        :value="shelfItem.value"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
              </td>
      ...
      <el-form-item>
        <el-button type="primary" @click="onSubmit('formSpec')">更新商品规格</el-button>
      </el-form-item>
    </el-form>

    <el-form class="demo-form-inline" ref="formRules" :rules="formRule" :model="formRules" label-width="160px"
      size="small">
      <el-col :span="24">
        <el-col :span="24">
          <div class="grid-content">
            <el-form-item label="商品详情：" prop="item_content">
              <!--  <el-input
                type="textarea"
                :autosize="{ minRows: 8, maxRows:8 }"
                placeholder="请输入商品详情"
      ...
      <el-col :span="24">
        <el-form-item>
          <el-button type="primary" @click="onSubmitDetail('formRules')">更新商品详情</el-button>
