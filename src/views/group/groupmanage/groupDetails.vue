<template>
  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            v-if="scope.row.real_status==3"
            type="text"
            @click="delGroup(scope.row.tid)"
          >移除此群</el-button>
          <el-button
            size="mini"
            v-else-if="scope.row.real_status==0"
            type="text"
    ...
    <pagination
      ...
      v-on:pagination="getChatroomUserList"
    />
  </div>
</template>
...
import { getChatroomUserList, updateRealChat } from "@/api/shopGoods"
...
export default {
  ...
  methods: {
    ...
    getChatroomUserList() {
      ...
      getChatroomUserList(this.listQuery).then(res => {
        if (res.code == 200) {
